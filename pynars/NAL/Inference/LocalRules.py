...
def revision(task: Task, belief: Task, budget_tasklink: Budget = None, budget_termlink: Budget = None):
    ...
    budget, *_ = Budget_revision(task.budget, truth1, truth2, truth, budget_tasklink=budget_tasklink,
                                 budget_termlink=budget_termlink)
    term = premise1.term
    # stamp: Stamp = deepcopy(task.sentence.stamp)
    # Stamp(Global.time, task.sentence.stamp.t_occurrence, None, (j1.stamp.evidential_base | j2.stamp.evidential_base))
...
def solution_question(task: Task, belief: Belief, budget_tasklink: Budget = None, budget_termlink: Budget = None):
    ...
    if answer is not None and question.answer_best is answer:
        ...
        # reward the belief
        budget_answer = Budget(Or(task.budget.priority, quality), task.budget.durability,
                               truth_to_quality(answer.truth))
        belief = Belief(Judgement(answer.term, answer.stamp, answer.truth), budget_answer)

        # de-prioritize the question
        task.budget.priority = min(1 - quality,
                                   task.budget.priority)
        # BUG: here, after setting the priority, the level of the task should change within a Bag.

    if answer is not None:
        # task.withdraw_brothers()  # the new one
        return belief
    else:
        return None
