...
def Budget_revision(budget_task: Budget, truth_task: Truth, truth_belief: Truth, truth_derived: Truth,
                    budget_tasklink: Budget = None, budget_termlink: Budget = None, replace = True,
                    replace_tasklink = True, replace_termlink = True):
    """
    budget_task (Budget):
    ...
    if not replace:
        budget_task = deepcopy(budget_task)
    ...
    if budget_tasklink is not None:
        if not replace_tasklink:
            budget_tasklink = deepcopy(budget_tasklink)
    ...
    if budget_termlink is not None:
        if not replace_termlink:
            budget_termlink = deepcopy(budget_termlink)
...
def Budget_backward_weak(truth_new_task: Truth, budget_tasklink: Budget, budget_termlink: Budget = None):
    return Budget_inference(w_to_c(1, Config.k) * truth_to_quality(truth_new_task), budget_tasklink, budget_termlink,
                            1.0)


def Budget_forward_compound(content: Term, truth_new_task: Truth, budget_tasklink: Budget,
                            budget_termlink: Budget = None):
    """
    Ref. OpenNARS 3.1.0 BudgetFunctions.java line 254~257.
    """
    return Budget_inference(truth_to_quality(truth_new_task), budget_tasklink, budget_termlink,
                            Config.complexity_unit if content is None else Config.complexity_unit * content.complexity)
...
def Budget_backward_weak_compound(content: Term, budget_tasklink: Budget, budget_termlink: Budget = None):
    return Budget_inference(w_to_c(1, Config.k), budget_tasklink, budget_termlink,
                            Config.complexity_unit * content.complexity)
...
def Budget_decay(budget: Budget, replace = True):
    ...
    q = budget.quality * Q
    d = budget.durability
    budget.priority = q + (p - q) * pow(d, 1.0 / (p * C + 1e-3))
